(ns deepfishery.actor-test
  (:require [clojure.test :refer [deftest is testing]]
            [deepfishery.actor :as actor]
...
(deftest commits-a-routine-catch-record-log
  (let [st (fresh-store)
        graph (actor/build-graph {:store st})
        request {:crew-id "crew-1" :op :log-catch-record :stake :low
                 :vessel-id "V-1" :species "tuna" :quantity-kg 500
                 :entry-text "no incidents during today's fishing trip"}
...
(deftest commits-a-within-threshold-supply-order
  (let [st (fresh-store)
        ...
        request {:crew-id "crew-1" :op :coordinate-supply-order :stake :low
                 :vessel-id "V-1" :items ["ice"] :cost 500}
...
(deftest interrupts-then-approves-an-over-threshold-supply-order-on-human-approval
  (let [st (fresh-store)
        graph (actor/build-graph {:store st})
        request {:crew-id "crew-1" :op :coordinate-supply-order :stake :low
                 :vessel-id "V-1" :items ["replacement nets"] :cost 9000}
